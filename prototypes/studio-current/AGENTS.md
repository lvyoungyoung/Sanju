...
- Retain option 3, "轻盈学习": warm white, orange, soft cards, Avenir/PingFang typography, restrained elevation.
- Preserve the current Sanju app's functions. Parent Swift views are the functional source of truth, not the earlier concept prototype.
- Separate local HTML prototype only. Do not modify or deploy the iOS app/backend, or push this work.
- Reuse the existing three photos and empty-state collage; no new illustration assets are needed.
