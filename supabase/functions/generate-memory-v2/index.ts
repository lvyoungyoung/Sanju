...
第二组 scene_and_feelings 必须是用户面对这张照片时最可能会脱口而出的三句自然英语。它不是第二组画面描述，而是帮助用户把自己的生活说出来。大胆根据画面推测最可能发生的场景、人物关系或感受；不需要反复说明这是推测，但不得编造图片无法支持的具体姓名、地点、时间、经历或事实。第三句不受前面“不要虚构对话”的限制：它是基于画面的假设性口语示例，不能把假设的对话写成真实发生过的事实。
第二组的三句必须按以下顺序各写一句，三句表达的角度必须明显不同：
1. 发生了什么：用第一人称或第一人称复数，说一个最可能发生的日常场景或动作，例如 "I had such a lovely time with my friends."。
2. 我当时的感受：自然说出看到或经历这个画面时的情绪、反应或氛围，例如 "This little moment made my whole day."。
3. 当时会对别人说什么：假设用户正处在照片里的场景中，写一句最自然、最贴合眼前情境、可以直接对别人说的英语，例如邀请、提问、请求、提醒或回应。句子要用到画面中的具体对象或活动，不要只写通用寒暄，也不要再写一遍自己的感受或事后的照片配文。例如咖啡场景可说 "Would you like to try a sip of my coffee?"，风景场景可说 "Could you take a photo of me with this view?"。直接输出用户会说的那一句，不要输出双方对话，不加说话人标签或额外引号，不要使用 I would say 等解释性开头；中文也直接翻译这句口语。
前两句优先使用 I 或 we，第三句可自然使用 you、we、祈使句或疑问句，不强制第一人称。像人会对朋友说、发照片时会配的日常英语。不要写成客观的物体清单，不要让三句只是同义改写，也不要使用空泛、放之四海皆准的鸡汤。
${englishLevel === "启蒙" ? '启蒙的生活表达也必须使用 3 到 6 个单词的超短句，上面的长句仅用于说明表达角度，不能模仿其长度和难度。可依次参考 I am at a party.、I am happy.、Come and sit with me.，但内容必须贴合当前照片。启蒙词汇和句长限制优先于表达层次和风格。' : '生活表达的“日常口语感”优先级高于用户选择的英语级别和语言风格：'}
