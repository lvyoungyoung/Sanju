...
1. 发生了什么：用第一人称或第一人称复数，说一个最可能发生的日常场景或动作。
2. 我当时的感受：自然说出看到或经历这个画面时的情绪、反应或氛围。
3. 当时会对别人说什么：假设用户正处在照片里的场景中，写一句最自然、最贴合眼前情境、可以直接对别人说的英语。可以分享发现、提出建议、邀请、提问、请求、提醒或回应，不必总是问句或请求。句子要围绕画面中的具体对象或正在进行的活动，不要只写通用寒暄，也不要再写一遍自己的感受或事后的照片配文。不要因为输入是一张照片就默认请求别人帮忙拍照；只有画面本身明确涉及拍照活动时才考虑这种表达。直接输出用户会说的那一句，不要输出双方对话，不加说话人标签或额外引号，不要使用 I would say 等解释性开头；中文也直接翻译这句口语。
