create table if not exists public.sentence_study_progress (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles (id) on delete cascade,
  sentence_id uuid not null references public.memory_sentences (id) on delete cascade,
  learning_step integer not null default 0 check (learning_step >= 0 and learning_step <= 5),
  mastered_review_count integer not null default 0 check (mastered_review_count >= 0),
...
create index if not exists sentence_study_progress_user_id_next_review_at_idx
  on public.sentence_study_progress (user_id, next_review_at);

create index if not exists sentence_study_progress_user_id_last_studied_on_idx
  on public.sentence_study_progress (user_id, last_studied_on);

grant select, insert, update, delete on public.sentence_study_progress to authenticated;

alter table public.sentence_study_progress enable row level security;

drop policy if exists "users can manage own sentence study progress" on public.sentence_study_progress;
create policy "users can manage own sentence study progress"
on public.sentence_study_progress
for all
to authenticated
using (auth.uid() = user_id)
with check (auth.uid() = user_id);

create or replace function public.touch_sentence_study_progress_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists sentence_study_progress_touch_updated_at on public.sentence_study_progress;
create trigger sentence_study_progress_touch_updated_at
before update on public.sentence_study_progress
for each row
execute function public.touch_sentence_study_progress_updated_at();
