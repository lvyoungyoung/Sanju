...
- `scene_and_feelings`：三句场景表达，依次表达发生了什么、当时的感受、当时会对别人说什么。第三句是贴合照片场景的单句假设性口语（如邀请、提问或请求），不是双方对话，也不声称对话真实发生过；允许有画面依据的场景推测，但不编造具体姓名、地点等细节。
- 三句场景表达不提供固定英文范句，避免套用模板；第三句也可以分享发现或提出建议，不默认请求拍照，只有画面本身明确涉及拍照活动时才考虑这种表达。
